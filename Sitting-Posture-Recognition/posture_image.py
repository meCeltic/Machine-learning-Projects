...
def process (input_image, params, model_params):
	...
	canvas1 = draw(input_image,all_peaks, position) #show the image.
	return canvas1 , position


def draw(input_image, all_peaks, position):
    canvas = cv2.imread(input_image)  # B,G,R order
    for i in range(18):
        for j in range(len(all_peaks[i])):
            cv2.circle(canvas, all_peaks[i][j][0:2], 4, colors[i], thickness=-1)
    
    # Adding text labels based on detected positions
    if position == 1:
        cv2.putText(canvas, "Hunchback", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    elif position == -1:
        cv2.putText(canvas, "Rest", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
    else:
        cv2.putText(canvas, "Straight", (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
if __name__ == '__main__': #main function of the program
	...
	if(vi == False):
	    ...
	    canvas, position= process('./sample_images\image.png', params, model_params)
	    showimage(canvas)
	    if (position == 1):
	    	print("Hunchback")
	    elif (position == -1):
